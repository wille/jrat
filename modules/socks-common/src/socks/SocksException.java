...
public class SocksException extends java.io.IOException {
    ...
    public SocksException(int errCode) {
        ...
        if ((errCode >> 16) == 0) {
            ...
            errString = errCode <= serverReplyMessage.length ? serverReplyMessage[errCode] : UNASSIGNED_ERROR_MESSAGE;
        } else {
            // Local error
            errCode = (errCode >> 16) - 1;
            errString = errCode <= localErrorMessage.length ? localErrorMessage[errCode] : UNASSIGNED_ERROR_MESSAGE;
        }
    }
