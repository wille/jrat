...
public class Socks5DatagramSocket extends DatagramSocket {
     ...
     */
    public Socks5DatagramSocket(int port, InetAddress ip) throws IOException {
     ...
     */
    public Socks5DatagramSocket(CProxy p, int port, InetAddress ip) throws IOException {
        super(port, ip);
        if (p == null)
        ...
        if (p.chainProxy != null)
            throw new SocksException(CProxy.SOCKS_JUST_ERROR, "Datagram Sockets do not support proxy chaining.");
	 ...
	 * 
	 * CProxy.setDefaultProxy(proxyHost,proxyPort); CProxy p =
	 * CProxy.getDefaultProxy(); p.addDirect("lux");
	 * 
	 * 
