...
import pro.jrat.client.packets.outgoing.Packet13OneRemoteScreen;
...
public class FrameRemoteScreenOld extends BaseFrame {

	private JPanel contentPane;
	public Slave slave;
	public boolean running = false;
	public static HashMap<Slave, FrameRemoteScreenOld> instances = new HashMap<Slave, FrameRemoteScreenOld>();
	public FrameRemoteScreenOld frame;
	...
	public static final void show(Slave sl) {
		FrameRemoteScreenOld frame = new FrameRemoteScreenOld(sl);
		frame.setVisible(true);
		//FrameMonitors dialog = new FrameMonitors(frame, sl);
		// TODO dialog.setVisible(true);
	}
	...
	public FrameRemoteScreenOld(Slave slave) {
		super();
		thread.start();
		this.slave = slave;
		instances.put(slave, this);

		setIconImage(Toolkit.getDefaultToolkit().getImage(FrameRemoteScreenOld.class.getResource("/icons/screen.png")));
		...
		lbl.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if (mouseInput()) {
					...
					sl.addToSendQueue(new Packet92MousePress(e.getX(), e.getY(), xButton, monitor));
				}
			}
			...
			public void mouseReleased(MouseEvent e) {
				if (mouseInput()) {
					...
					sl.addToSendQueue(new Packet93MouseRelease(e.getX(), e.getY(), xButton, monitor));
				}
			}
		});
		...
		btnStartCapture.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				running = true;
				clearSize();
				sl.addToSendQueue(new Packet12RemoteScreen(getPercentSize(), quality, monitor));
				btnRequestOne.setEnabled(false);
				btnStartCapture.setEnabled(false);
				btnStop.setEnabled(true);
			}
		});
		btnStartCapture.setIcon(new ImageIcon(FrameRemoteScreenOld.class.getResource("/icons/start.png")));
		...
		btnRequestOne.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				running = true;
				clearSize();
				sl.addToSendQueue(new Packet13OneRemoteScreen(getPercentSize(), quality, monitor));
			}
		});

		btnStop = new JButton("");
		btnStop.setEnabled(false);
		btnStop.setIcon(new ImageIcon(FrameRemoteScreenOld.class.getResource("/icons/stop.png")));
		...
		toolBar.add(btnStop);
		btnRequestOne.setIcon(new ImageIcon(FrameRemoteScreenOld.class.getResource("/icons/camera.png")));
		...
		btnRecord1.setIcon(new ImageIcon(FrameRemoteScreenOld.class.getResource("/icons/record.png")));
		...
		btnSave_1.setIcon(new ImageIcon(FrameRemoteScreenOld.class.getResource("/icons/save.png")));
		...
		btnKeyboard.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				FrameRemoteScreenKeyboard frame = FrameRemoteScreenKeyboard.instances.get(FrameRemoteScreenOld.this);
				if (frame == null) {
					//frame = new FrameRemoteScreenKeyboard(FrameRemoteScreenOld.this);
		...
		btnKeyboard.setIcon(new ImageIcon(FrameRemoteScreenOld.class.getResource("/icons/keyboard.png")));
		toolBar.add(btnKeyboard);

		tglbtnMouse = new JToggleButton("Mouse");
		tglbtnMouse.setSelected(true);
		tglbtnMouse.setIcon(new ImageIcon(FrameRemoteScreenOld.class.getResource("/icons/mouse.png")));
		toolBar.add(tglbtnMouse);

		tglbtnMove = new JToggleButton("");
		tglbtnMove.setIcon(new ImageIcon(FrameRemoteScreenOld.class.getResource("/icons/arrow_move_icon.png")));
